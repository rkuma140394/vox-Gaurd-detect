  ...
  if (isLoading) {
    return (
      <div className="mt-8 p-10 bg-slate-900 border border-slate-700 rounded-3xl text-center shadow-2xl relative overflow-hidden">
        ...
        <p className="text-white font-bold tracking-tight mb-2">Real-Time Forensic Analysis</p>
        <p className="text-slate-400 text-xs font-mono animate-pulse">Optimized Engine (Low-Latency Mode)...</p>
      </div>
    );
  }
  ...
  return (
    <div className="mt-8 animate-in fade-in zoom-in-95 duration-500">
      <div className={`p-8 rounded-3xl border-2 shadow-2xl bg-white relative overflow-hidden ${isAI ? 'border-rose-100' : 'border-emerald-100'}`}>
        ...
        {/* Real-Time Badge */}
        <div className="absolute top-4 right-4 flex items-center space-x-1.5 bg-emerald-50 px-3 py-1 rounded-full border border-emerald-100">
           <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse"></div>
           <span className="text-[9px] font-black text-emerald-600 uppercase tracking-tighter">Real-Time Optimized</span>
        </div>

        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            <p className="text-[10px] uppercase font-black tracking-widest text-slate-400 mb-2">Analysis Verdict</p>
          ...
          <div className="w-full md:w-auto">
             <div className="flex justify-between items-end mb-1">
                <p className="text-[10px] uppercase font-black tracking-widest text-slate-400">Certainty</p>
        ...
        <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 relative">
          ...
          <p className="text-slate-700 text-sm leading-relaxed font-semibold pl-6 relative z-10">
            {result.explanation}
          </p>
        </div>
        ...
        <div className="mt-8 flex flex-wrap items-center justify-between gap-4 text-[10px] font-black text-slate-400 uppercase tracking-widest border-t border-slate-50 pt-6">
            ...
            <span className="flex items-center"><i className="fas fa-bolt mr-2 text-indigo-400"></i> Gemini 3 Flash Latency Optimized</span>
        </div>
      </div>
    </div>
  );
